...
export class AcenderVelasUseCase {
    ...
    async execute(jogador: Jogador) {
        const mensagem = jogador.acenderVelas();

        if (!jogador.id) {
            throw new Error('Erro ao buscar o id do Jogador!');
        }

        const atualizou = await this.repository.acenderVelas(jogador.id);

        if (!atualizou) {
            throw new Error('Ocorreu um erro ao tentar acender as velas!');
        }
